...
public:
  ...
  Instruction () = default;
  Instruction (Type type, size_t id, Instruction* prev, Instruction* next, Opcode opcode, RetType ret_type, BasicBlock* bb) : InstructionOrValue(type), id(id), prev(prev), next(next), opcode(opcode), ret_type(ret_type), bb(bb) {};
  virtual ~Instruction() = default;

  BasicBlock* get_bb() const { return bb; }
  size_t get_id() const { return id; }
  Opcode get_opcode() const { return opcode; }
  RetType get_ret_type() const { return ret_type; }
  ...
  void set_bb(BasicBlock* other) {bb = other;}
  void set_id(size_t other) {id = other;}
  void set_next(Instruction* other) {next = other;}
  void set_prev(Instruction* other) {prev = other;}
...
public:
  Instruction* get_first_inst () { return first_inst;}
  Instruction* get_last_inst () { return last_inst;}
...
public:
  AssignInst (Type type, size_t id, Instruction* prev, Instruction* next, Opcode opcode, RetType ret_type, BasicBlock* bb) : Instruction (type, id, prev, next, opcode, ret_type, bb) {}
};

class CondInst : public Instruction {
public:
  CondInst (Type type, size_t id, Instruction* prev, Instruction* next, Opcode opcode, RetType ret_type, BasicBlock* bb, CondCode cond_code) : Instruction (type, id, prev, next, opcode, ret_type, bb), cond_code(cond_code) {}
...
public:
  PhiInst (Type type, size_t id, Instruction* prev, Instruction* next, Opcode opcode, RetType ret_type, BasicBlock* bb) : Instruction (type, id, prev, next, opcode, ret_type, bb) {}
